...
const MailMerge = () => {
    ...
    let hostUrl: string =
        "http://localhost:62871/api/documenteditor/";
...
container.current!.documentEditor.open(JSON.stringify(defaultDocument));
        ...
        let listviewElement = document.getElementById("listview");
        if (listviewElement) {
            listviewElement.addEventListener("dragstart", (event: any): void => {
        ...
        if (containerElement) {
            containerElement.addEventListener("dragover", (event: any): void => {
                event.preventDefault();
            });

            // Drop Event for document editor element
            containerElement.addEventListener("drop", (e: any) => {
        ...
        let listDivElement: HTMLElement | null = document.getElementById("listview");
        if (listDivElement) {
    ...
    const mergeDocument = (): void => {
        container.current!.documentEditor.saveAsBlob("Docx").then((blob: Blob) => {
            ...
            fileReader.onload = (): void => {
                ...
                let baseUrl: string =
                    "http://localhost:62871/api/documenteditor/MailMerge";
    ...
    const showHideWaitingIndicator = (show: boolean) => {
        ...
        if (waitingPopUp) waitingPopUp.style.display = show ? "block" : "none";
        if (inActiveDiv) inActiveDiv.style.display = show ? "block" : "none";
    };
    const showInsertFielddialog = (container: any): void => {
    ...
    return (
        <div className="mail_merge-control-pane">
            <div className="mail_merge-control-section">
                <div className="mail_merge-left-panel">
                    <div className="mail_merge-left-panel-header">
                        <h5>
                            <label>Select Field to Insert</label>
                        </h5>
                    </div>
                    <div id="listview" className="mail_merge-field-list"></div>
                </div>
                <div className="mail_merge-right-panel">
                    <div id="documenteditor_titlebar" className="mail_merge-toolbar"></div>
                    <div className="mail_merge-editor-container">
                        <DocumentEditorContainerComponent
                            id="container"
                            ref={container}
                            className="mail_merge-document-editor"
                            style={{ display: "block" }}
                            height={"100%"}
                            serviceUrl={hostUrl}
                            enableToolbar={true}
