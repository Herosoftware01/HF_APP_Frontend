...
const Bundel = () => {
  ...
  useEffect(() => {
    buildRows();
  }, [pendingApi, completeApi, searchUnit, filterStart, filterEnd]);
  useEffect(() => {
    buildReceivePending();
  }, [pendingApi, completeApi, searchUnit, filterStart, filterEnd]);
  useEffect(() => {
    buildModalRows();
  }, [pendingApi, completeApi, filterStart, filterEnd, selectedUnit]);
  useEffect(() => {
    buildAllUnitModalCounts();
  }, [pendingApi, completeApi, filterStart, filterEnd, allUnits]);
  ...
  const buildRows = () => {
    ...
    completeApi.forEach((item) => {
      ...
      const mst = Number(
        pick(item, 'total_bundles', 'totmastbdl', 'mst_count') || 0
      );
      const sub = Number(pick(item, 'pcs_count', 'totbdl', 'sub_count') || 0);
      const job =
        pick(item, 'job_no', 'jobno', 'job') ||
  ...
  const buildReceivePending = () => {
    ...
    pendingApi.forEach((item) => {
      ...
      const sub = Number(pick(item, 'totbdl') || 0);
      const pcs = Number(
        pick(item, 'pcs_count') || pick(item, 'ordsamid') || 0
      );
      const job = pick(item, 'jobno') || '-';
    ...
    const summaries = sortedKeys.map((unitName) => {
      ...
      return {
        ...
        jobs: [
          ...new Set(rows.map((r) => r.job_no).filter((j) => j !== '-')),
        ].join(', '),
      };
    });
  ...
  const buildAllUnitModalCounts = () => {
    ...
    completeApi.forEach((item) => {
      ...
      const pcsMatch = Number(
        (matched && pick(matched, 'ordsamid')) ||
          pick(item, 'total_bundles') ||
          0
      );
      const subMatch = Number(
        (matched && pick(matched, 'totbdl')) || pick(item, 'pcs_count') || 0
      );
  ...
  const buildModalRows = () => {
    ...
    completeApi.forEach((item) => {
      ...
      const rDateISO = parseDate(
        pick(item, 'r_date') || (matched && pick(matched, 'r_dt'))
      );
      const mst = Number(pick(item, 'total_bundles') || 0);
      const sub = Number(pick(item, 'pcs_count') || 0);
      const jobNo =
        pick(item, 'job_no') || (matched && pick(matched, 'jobno')) || '-';
      const bid = (matched && pick(matched, 'b_id')) || pick(item, 'id') || '-';
      const masterBdl = Number(
        (matched && pick(matched, 'totmastbdl')) || mst || 0
      );
  ...
  // TABLE HELPERS
  // ─────────────────────────────────────────────────────────────
  const rowSpans = (rows, key) =>
    rows.map((r, i) => {
      if (i > 0 && rows[i][key] === rows[i - 1][key]) return 0;
      let c = 1;
      for (let j = i + 1; j < rows.length; j++) {
        if (rows[j][key] === r[key]) c++;
        else break;
      }
      return c;
    });
  ...
  const renderRows = (rows) => {
    if (!rows.length)
      return (
        <tr>
          <td
            colSpan={modalFilter === 'complete' ? 10 : 9}
    ...
    return sortedRows.map((row, i) => (
      <tr
        key={i}
        className="border-b border-slate-200/60 hover:bg-slate-50/80 text-center text-[11px]"
      >
        {rs[i] > 0 && (
          <td
            rowSpan={rs[i]}
            className="p-3 border border-gray-300 bg-blue-50/40 font-bold text-blue-700 align-middle"
          >
            {row.receiveDate}
          </td>
        )}
        {us[i] > 0 && (
          <td
            rowSpan={us[i]}
            className="p-3 border border-gray-300 bg-white font-semibold text-slate-800 align-middle"
          >
            {row.unit}
          </td>
        )}
        ...
        <td className="p-3 font-mono text-slate-600 border border-gray-300">
          {row.bundle}
        </td>
        <td className="p-3 font-mono text-slate-600 border border-gray-300">
          {row.pcs_count}
        </td>
        {modalFilter === 'complete' && (
          <td className="p-3 text-emerald-600 font-semibold border border-gray-300">
            {row.scannedDate}
          </td>
        )}
        <td className="p-3 font-bold text-slate-600 border border-gray-300">
          {row.ageing}
        </td>
        <td className="p-3 border border-gray-300">
          <span
            className={`px-2.5 py-1 rounded-md text-[9px] font-bold uppercase tracking-wider ${riskBadge(row.risk)}`}
          >
            {row.risk}
          </span>
        </td>
  ...
  const getRpSortedRows = (rows) => {
    ...
    return [...rows].sort((a, b) => {
      if (a._rdtISO !== b._rdtISO)
        return new Date(a._rdtISO) - new Date(b._rdtISO);
    ...
    <div className="p-8 bg-[#fdfdfd] min-h-screen font-sans text-slate-900">
      <div className="max-w-screen-2xl mx-auto">
        ...
        <div className="mb-6 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">
              Unit Bundle Tracking Dashboard
            </h1>
            ...
            <p className="text-slate-500 text-sm mt-3 font-medium tracking-wide uppercase">
              Operational Throughput Control
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-3 bg-white shadow-sm border border-slate-200 p-2 rounded-2xl">
            <div className="flex flex-col gap-0.5">
              ...
              <div className="flex items-center bg-slate-50 rounded-xl px-3 py-1">
                <span className="text-slate-400 text-[11px] font-bold mr-2 uppercase">
                  From:
                </span>
                <input
                  type="date"
        ...
        <div className="mb-4 flex items-center gap-3 text-[10px] font-bold uppercase">
          ...
          <span className="px-2.5 py-1 rounded-md bg-yellow-100 text-yellow-700">
            Normal (0–2d)
          </span>
          <span className="px-2.5 py-1 rounded-md bg-orange-100 text-orange-600">
            Risk (2–5d)
          </span>
          <span className="px-2.5 py-1 rounded-md bg-red-100 text-red-600">
            High Risk (&gt;5d)
          </span>
          <span className="px-2.5 py-1 rounded-md bg-emerald-100 text-emerald-600">
            Complete
          </span>
        </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-6 gap-3">
              {receivePendingSummary.map((s, i) => (
                ...
                >
                  ...
                  <p className="text-3xl font-black text-amber-600 group-hover:scale-105 transition-transform">
                    {s.totalCount}
                  </p>
            ...
            return (
              ...
              >
                ...
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="bg-slate-100 px-3 py-1 rounded-full text-[9px] font-black text-slate-500 uppercase">
                    Live
                  </div>
                </div>
                ...
                <div className="mb-1">
                  <p className="text-[8px] font-black text-slate-300 uppercase tracking-widest mb-2">
                    ...
                    <span className="text-blue-400">
                      ({fmt(filterStart)}
                      {filterStart !== filterEnd ? ` → ${fmt(filterEnd)}` : ''})
                    </span>
                  </p>
                </div>
                <div className="flex flex-col gap-3">
                  <div className="bg-red-50/50 border border-red-100 p-4 rounded-2xl">
                    <div className="flex justify-between items-center mb-1">
                      <p className="text-red-400 text-[9px] font-bold uppercase">
                        Pending
                      </p>
                  ...
                  <div className="bg-emerald-50/50 border border-emerald-100 p-4 rounded-2xl">
                    <div className="flex justify-between items-center mb-1">
                      <p className="text-emerald-400 text-[9px] font-bold uppercase">
                        Complete
                      </p>
        ...
        {receivePendingUnit &&
          (() => {
            const rows = receivePendingRows.filter(
              (r) => r.unit === receivePendingUnit
            );
            ...
            const normalCount = sorted.filter(
              (r) => r.risk === 'Normal'
            ).length;
                ...
                <div className="bg-white rounded-3xl w-full max-w-6xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl border border-slate-200">
                  ...
                  <div className="px-8 py-5 flex justify-between items-center bg-amber-50/50 border-b border-amber-200">
                    <div className="flex items-center gap-5">
                      <button
                        onClick={() => setReceivePendingUnit(null)}
                      ...
                      <div>
                        ...
                        <p className="text-amber-600 font-bold text-[10px] uppercase tracking-widest mt-1">
                          Receive Pending — {fmt(filterStart)} to{' '}
                          {fmt(filterEnd)}
                        </p>
                      </div>
                    </div>
                    ...
                    <button
                      onClick={() => setReceivePendingUnit(null)}
                      className="h-10 w-10 rounded-full hover:bg-red-50 hover:text-red-500 flex items-center justify-center font-bold text-slate-400"
                    >
                      ✕
                    </button>
                  </div>
                  ...
                  <div className="flex items-center gap-3 px-6 py-3 bg-slate-50 border-b border-slate-200 flex-wrap">
                    ...
                    <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-red-100 border border-red-200">
                      ...
                      <span className="text-[15px] font-black text-red-700">
                        {highRiskCount}
                      </span>
                    </div>
                    ...
                    <table className="w-full border-separate border-spacing-0">
                      <thead className="sticky top-0 bg-white">
                        <tr className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">
                          <th className="p-4 border border-gray-200 bg-white/90">
                            Received Date
                          </th>
                          <th className="p-4 border border-gray-200 bg-white/90">
                            Unit
                      ...
                      <tbody>
                        {sorted.length === 0 ? (
                          ...
                          sorted.map((row, i) => (
                            <tr
                              key={i}
                              className="border-b border-slate-200/60 hover:bg-amber-50/40 text-center text-[11px]"
                            >
                              {rs[i] > 0 && (
                                <td
                                  rowSpan={rs[i]}
                                  className="p-3 border border-gray-300 bg-amber-50/60 font-bold text-amber-700 align-middle"
                                >
                                  {row.receiveDate}
                                </td>
                              )}
                              {rs[i] > 0 && (
                                <td
                                  rowSpan={rs[i]}
                                  className="p-3 border border-gray-300 bg-blue-50/60 font-bold text-blue-700 align-middle"
                                >
                                  {row.unit}
                                </td>
                              )}
                              ...
                              <td className="p-3 text-slate-500 border border-gray-300">
                                {row.sub}
                              </td>
                              <td className="p-3 font-mono text-slate-600 border border-gray-300">
                                {row.pcs_count}
            ...
            <div className="bg-white rounded-3xl w-full max-w-6xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl border border-slate-200">
              ...
              <div className="flex-1 overflow-auto bg-white">
                <table className="w-full border-separate border-spacing-0">
                  <thead className="sticky top-0 bg-white">
                    <tr className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">
                      <th className="p-4 border-x bg-white/90">
                        Received Date
                      </th>
